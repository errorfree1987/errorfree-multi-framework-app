...
def main():
    ...
    # --- Tenant namespace (D3) validation display ---
    try:
        ns_reviews = tenant_namespace("reviews")
        ns_exports = tenant_namespace("exports")
    except Exception:
        ns_reviews = "(tenant_namespace() not available)"
        ns_exports = "(tenant_namespace() not available)"

    st.markdown("---")
    if not is_zh:
        st.caption("Tenant namespace (reviews):")
        st.code(ns_reviews)
        st.caption("Tenant namespace (exports):")
        st.code(ns_exports)
    else:
        st.caption("租戶命名空間（reviews）：")
        st.code(ns_reviews)
        st.caption("租戶命名空間（exports）：")
        st.code(ns_exports)
