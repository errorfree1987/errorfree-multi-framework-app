...
def render_followup_history_chat(followup_history: List, lang: str):
    """Follow-up history display, collapsed per question to keep the page short."""
    if not followup_history:
        st.caption("No follow-up history yet." if lang == "en" else zh("目前尚無追問紀錄。", "目前尚无追问记录。"))
        return

    with st.expander(
        "Follow-up history (Chat view)" if lang == "en" else zh("追問歷史（對話排列）", "追问历史（对话排列）"),
        expanded=False,
    ):
        for i, (q, a) in enumerate(followup_history, start=1):
            q_one_line = " ".join((q or "").split())
            if len(q_one_line) > 80:
                q_one_line = q_one_line[:80] + "…"
            label = f"{i}. {q_one_line}" if q_one_line else f"{i}. (follow-up)"
        ...
        if is_guest and current_state.get("download_used"):
            ...
            with st.expander("Download"):
                ...
                if fmt.startswith("PDF") or fmt.startswith("PowerPoint"):
                    ...
                    filename = f"Error-Free® IER + {selected_key} + {now_str}.docx"
