...
def main():
  ...
  # --- Sidebar UI (NO "with st.sidebar" to avoid indentation issues) ---
ui_lang = st.session_state.get("lang", "en")
ui_zhv = st.session_state.get("zh_variant", "tw")
is_zh = (ui_lang == "zh")

st.sidebar.header(
    "🧭 " + ("Error-Free® Intelligence Engine" if ui_lang == "en" else "零錯誤智能引擎")
)
...
# --- Tenant namespace (D3) verification ---
st.sidebar.caption(f"Namespace(root): {tenant_namespace()}")
st.sidebar.caption(f"Namespace(reviews): {tenant_namespace('reviews')}")
st.sidebar.caption(f"Namespace(exports): {tenant_namespace('exports')}")
