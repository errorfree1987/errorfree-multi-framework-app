...
def build_full_report_pdf(lang: str):
    ...
    try:
        ...
        return None, "目前伺服器尚未安裝 reportlab，無法輸出 PDF。請在 requirements.txt 中加入 reportlab 後重新部署。"
...
def main():
    ...
    with st.sidebar:
        ...
        if analysis_active:
            if current_lang == "zh":
                ...
                st.caption("Current language: English (locked for this analysis).")
            lang = current_lang
        else:
            if current_lang == "en":
                st.markdown("### Language")
    ...
    with st.sidebar:
        ...
        if st.session_state.user_role is not None:
            ...
            if lang == "zh":
                ...
                if daily_limit is None:
                    ...
                    st.caption(f"今日已用次數：{today_usage} / {daily_limit}，剩餘：{remaining}")
            else:
                st.caption(f"Current model: **{model_name}**")
                if daily_limit is None:
                    st.caption(f"Today used: {today_usage} (no limit)")
                else:
                    st.caption(f"Today used: {today_usage} / {daily_limit}; remaining: {remaining}")
        ...
        if st.session_state.user_role == "admin":
            ...
            if lang == "zh":
                ...
                st.write(
                    f"Session 使用日期 = {st.session_state.usage_date}, 今日次數 = {st.session_state.usage_count}"
                )
                ...
                st.write(
                    f"Session usage_date = {st.session_state.usage_date}, today usage_count = {st.session_state.usage_count}"
                )
    ...
    if analysis_locked:
        if lang == "zh":
            ...
            st.info(
                ...
                "To avoid overwriting it, the button is locked. "
                "To analyze a new document, click \"Clear current analysis\" in the sidebar."
            )
    ...
    if run_clicked and not analysis_locked:
        ...
        with st.spinner("正在進行 AI 分析..." if lang == "zh" else "Running AI analysis..."):
            ai_output = run_llm_analysis(framework_key, lang, text_to_use, model_name)
    ...
    if st.session_state.last_analysis_output:
        ...
        if truncated:
            if lang == "zh":
                ...
                st.caption(
                    f"(Note: The document is long. Only the first {max_chars:,} characters were analyzed "
                    "to keep stability and cost under control. For very long documents, consider splitting them.)"
                )
    ...
    if st.session_state.last_analysis_output:
        ...
        if lang == "zh":
            ...
            st.markdown(
                "Based on the **analysis above** and the **original document**, you can ask follow-up questions "
                "to the Error-Free® consultant here (prioritization, suggested wording, examples, practical steps…)."
            )
        ...
        if lang == "zh":
            followup_question = st.text_area("請輸入你想追問的問題（與本次文件與分析相關）")
            ask_label = "送出提問"
            warn_need_q = "請先輸入一個問題。"
            warn_need_login = "請先登入授權帳號，才可使用後續提問功能。"
        else:
            followup_question = st.text_area("Enter your follow-up question (related to this document and analysis)")
        ...
        if st.button(ask_label):
            if st.session_state.user_role is None:
                ...
                st.session_state.followup_history.append((followup_question.strip(), answer))

                st.markdown("---")
                idx = len(st.session_state.followup_history)
                if lang == "zh":
                    st.markdown(f"**Q{idx}：{followup_question.strip()}**")
                    st.markdown(f"**A{idx}：**")
                else:
                    st.markdown(f"**Q{idx}: {followup_question.strip()}**")
                    st.markdown(f"**A{idx}:**")
        ...
        # 下載報告（txt / docx / pdf）
        full_report_txt = build_full_report(lang)
        if full_report_txt:
            st.markdown("---")
            if lang == "zh":
                st.markdown("### 下載完整報告")
            ...
            st.download_button(
                ...
                file_name="errorfree_full_report.txt",
                mime="text/plain",
            )

            docx_bytes = build_full_report_docx(lang)
            st.download_button(
                label="Word (.docx)",
                data=docx_bytes,
                file_name="errorfree_full_report.docx",
                mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
            )

            pdf_bytes, pdf_err = build_full_report_pdf(lang)
            if pdf_bytes is not None:
                st.download_button(
                    label="PDF",
                    data=pdf_bytes,
                    file_name="errorfree_full_report.pdf",
                ...
                if lang == "zh":
                    ...
                    st.info(pdf_err or "PDF generation is not available on this server.")
